...
use colby_core::{
	abi::{Bodies, Body, BodyId, Shape, ShapeKind, TraceInfo, TraceResult},
	glam::{Mat3, Vec3},
};
...
///
/// @param bodies - the table to trace against
/// @param simulation - the baked collision meshes, for their bounds
...
pub(crate) fn swept(bodies: &Bodies, simulation: &Simulation, info: &TraceInfo) -> TraceResult {
	let grow = info.extents.abs();

	nearest(bodies, info, |id, body| {
		let (low, high) = world_bounds(body, simulation.collider(id))?;
		let (low, high) = (low - grow, high + grow);

		let direction = info.end - info.start;
		let inside = contains(info.start, low, high);
		let (fraction, normal) = slab(info.start, direction, low, high)?;

		Some(Hit {
			fraction,
			normal: against(normal, direction),
			started_solid: inside,
			ended_solid: contains(info.end, low, high),
		})
	})
}
...
fn slab(origin: Vec3, direction: Vec3, low: Vec3, high: Vec3) -> Option<(f32, Vec3)> {
	...
	if sign == 0.0 {
		...
		return Some((enter, -direction.normalize_or(Vec3::Y)));
	}
