...
///
/// Ten spans of hardware, two of recording, five of simulation and one of
/// particles.
const ROWS: usize = 18;
...
/// third time, `gpu lamps` a fourth and `gpu prepass` a fifth.
const STEP_ROW: usize = 16;

/// Which row is the particles.
const SPARKS_ROW: usize = 17;
...
impl Live {
	...
	pub(crate) fn hardware(&mut self, passes: [Option<Duration>; 10], count: u32) {
...
///
/// The hardware spans first, then what this thread spent recording them, then
/// what the step underneath cost - which is the order a frame actually happens
/// in, read from the outside in.
///
/// `pub(crate)` because the editor's pane shows the same rows, and a panel
...
mod tests {
	...
	fn the_two_halves_of_a_frame_arrive_at_two_rates_and_both_land() {
		...
		assert_eq!(live.profile().parts[3].mean, Some(Duration::from_micros(800)));
		assert_eq!(live.profile().parts[3].name, "gpu scene", "in the fourth row");
	...
	#[cfg(feature = "editor")]
	#[test]
	fn the_live_table_and_the_printed_one_name_the_same_parts() {
