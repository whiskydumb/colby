...
pub enum Pass {
	...
	/// The pass before the scene: every solid thing in view drawn a second
	/// time, writing its depth, its normal and its roughness rather than a
	...
	/// the pass before the scene wrote: a march and an average at half the
	/// picture on each axis, and one pass back to its whole size. None at all
	/// in a frame nothing asks for it in. @ref
	...
	/// The light a haze sends towards the eye: a march and an average at half
	/// the picture on each axis, and one pass putting the result over the
	/// picture with what the air takes out of it. None at all while a world's
	/// air is clear. @ref [`haze`](crate::haze).
	Haze,
